...
void PickByColor::ReadActionParameters()
{
	...
	for (int i = 0; i < pManager->getFigCount(); i++) 
	{
		...
		if (Fig->GetGfxInfo().isFilled && !(dynamic_cast<CFigure*>(Fig))) //counts color occurance.
		{
			if (Fig->GetGfxInfo().FillClr == BLACK)
				clrs[0]++;
			else if (Fig->GetGfxInfo().FillClr == WHITE)
...
void PickByColor::Execute(bool read)
{
	Output* pOut = pManager->GetOutput();
	...
	if (no_colors > 1)
	{
		...
		if (Fig->GetGfxInfo().isFilled && !(dynamic_cast<CFigure*>(Fig)))
		{
			if (Fig->GetGfxInfo().FillClr == BLACK)
			{
		...
		while (picked_color_no > 0)
		{
			...
			if (P.y > UI.ToolBarHeight || P.x > (UI.MenuItemWidth * PLAY_ITM_COUNT))
			{
				...
				if (clickedFig != NULL)
				{
					...
					if ((!(Fig->GetGfxInfo().isFilled) && !(dynamic_cast<CFigure*>(Fig))) && (!(clickedFig->GetGfxInfo().isFilled) && !(dynamic_cast<CFigure*>(clickedFig))))
					{
						PrntScore(1);
						clickedFig->Hide();
					...
					else if ((dynamic_cast<CFigure*>(clickedFig)) && clickedFig->GetGfxInfo().DrawClr == AssignColor(Fig))
					{
						PrntScore(1);
						clickedFig->Hide();
						pManager->UpdateInterface();
					...
					else if (!(dynamic_cast<CFigure*>(clickedFig)) && clickedFig->GetGfxInfo().isFilled && clickedFig->GetGfxInfo().FillClr == AssignColor(Fig))
					{
						PrntScore(1);
						clickedFig->Hide();
						pManager->UpdateInterface();
...
bool PickByColor::CanBeRecorded() const
{
	return false;
}